...
| `python scripts/fetch_recruit_jds.py search --keyword 后台` | `/position/searchPosition` | POST | 官网岗位列表（岗位名/工作地/招聘类型/post_id） |
...
4. 仅当 MCP 和官网脚本均不可用或无命中时，再引导 `https://join.qq.com/post.html`
...
```
[优先运行 scripts/fetch_recruit_jds.py search/detail/match 从官网接口获取真实岗位；仅受控内测且 JD MCP 可用时，才用 campus-recruit-jd-qa 辅助检索]
[用表格列出 3-5 个与同学简历/背景最符合的真实校招岗位]
